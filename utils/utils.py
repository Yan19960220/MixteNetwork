...
def read_sits(filename, delimiter=','):
...
def read_all_datasets(data_dir, archive_name):
    datasets_dict = {}
    ...
    if archive_name == 'UCRArchive_2018':
        for dataset_name in DATASET_NAMES:
            file_name = "{}/{}/{}/{}".format(data_dir, archive_name, dataset_name, dataset_name)

            x_train, y_train = read_ucr(f"{file_name}_TRAIN.tsv")
            x_test, y_test = read_ucr(f"{file_name}_TEST.tsv")
        ...
        for dataset_name in utils.constants.dataset_names_for_archive[archive_name]:
            root_dir_dataset = data_dir + '/archives/' + archive_name + '/' + dataset_name + '/'
...
def read_sits_xps(root_dir):
    ...
    x_test, y_test = read_sits(path_to_test)
    ...
    for subdir, dirs, files in os.walk(path_to_data):
        for file_name in files:
            ...
            file_type = arr[1]
            if file_type == 'csv':
                x_train, y_train = read_sits(subdir + '/' + file_name)
